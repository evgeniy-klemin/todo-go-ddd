...
var ItemWhere = struct {
	...
	ID:        whereHelperstring{field: "`item`.`id`"},
	Name:      whereHelperstring{field: "`item`.`name`"},
	Position:  whereHelperint{field: "`item`.`position`"},
	Done:      whereHelperbool{field: "`item`.`done`"},
...
func FindItem(ctx context.Context, exec boil.ContextExecutor, iD string, selectCols ...string) (*Item, error) {
	...
	query := fmt.Sprintf(
		"select %s from `item` where `id`=?", sel,
	)
...
func (o *Item) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	...
	if !cached {
		...
		cache.query = buildUpsertQueryMySQL(dialect, "`item`", update, insert)
		cache.retQuery = fmt.Sprintf(
			"SELECT %s FROM `item` WHERE %s",
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to upsert for item")
	}
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to retrieve unique values for item")
	}
...
func ItemExists(ctx context.Context, exec boil.ContextExecutor, iD string) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from `item` where `id`=? limit 1)"
